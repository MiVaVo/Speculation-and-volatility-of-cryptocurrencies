...
from src.load_data import configs
from src.load_data.functional import *


def load_data_from_quandl():
    [create_folder(i) for i in [configs.data_dir, configs.datasets_created_python]]
    ...
    btc_usd_datasets_price = merge_dfs_on_column(list(exchange_data.values()), list(exchange_data.keys()),
                                                 'Weighted Price')
    btc_usd_datasets_volume = merge_dfs_on_column(list(exchange_data.values()), list(exchange_data.keys()),
                                                  'Volume (BTC)')
    ...
    btc_outst = pd.read_csv('../../data/total-bitcoins.csv', header=None)
    ...
    datelist = pd.date_range(btc_outst.iloc[0, 0],
                             periods=(btc_outst.iloc[-1, 0] - btc_outst.iloc[0, 0]).days + 1).tolist()
    dateframe = pd.DataFrame(datelist)
    dateframe.columns = ['date']
    ### NO DEVISION BY 2 BECAUSE IT IS SUPPLY !!!
    bts_outst_int = (
        pd.merge(dateframe, btc_outst, how='outer', on='date').set_index('date')).interpolate().reset_index()
    ...
    btc_quandl.to_csv(configs.datasets_created_python + '/' + 'btc_quandl.csv', header=True, index=True)
    ...
    df_to_regress = pd.merge(btc_quandl.reset_index(), btc_usd_datasets_new[['btc_tot', 'turnover', 'date', 'price']],
                             how='left', on='date')
    df_to_regress.head()
    ...
    df_to_regress.to_csv(configs.datasets_created_python + '/' + 'df_to_regress.csv', header=True, index=False)
    ###
